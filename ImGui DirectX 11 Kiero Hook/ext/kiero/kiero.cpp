...
kiero::Status::Enum kiero::bindFunction(void* _target, void** _original, void* _function)
{
	assert(_target != NULL && _original != NULL && _function != NULL);
	...
	// Create and enable hook on the raw function address
	if (MH_CreateHook(_target, _function, _original) != MH_OK)
		return Status::UnknownError;

	if (MH_EnableHook(_target) != MH_OK)
	{
	...
	return Status::Success;
#else
	(void)_target; (void)_original; (void)_function;
	return Status::NotSupportedError;
#endif
}

kiero::Status::Enum kiero::resolveModuleHandle(const char* _moduleName, void** _outBuffer) {
	HMODULE lib = ::GetModuleHandleA(_moduleName);
	if (lib == NULL)
		return Status::ModuleNotFoundError;
	*_outBuffer = lib;
}

kiero::Status::Enum kiero::bindImport(const char* _moduleName, const char* _procName, void** _original, void* _function)
{
	assert(_moduleName != NULL && _procName != NULL && _original != NULL && _function != NULL);

	// Get module handle and resolve the exported function address
	HMODULE lib = ::GetModuleHandleA(_moduleName);
	if (lib == NULL)
		return Status::ModuleNotFoundError;

	void* target = (void*)::GetProcAddress(lib, _procName);
	if (target == NULL)
		return Status::UnknownError;

	return bindFunction(target, _original, _function);
}

kiero::Status::Enum kiero::unbindFunction(void* _target)
{
	assert(_target != NULL);

#if KIERO_USE_MINHOOK
	// Keep consistent with other functions: require library initialized
	if (g_renderType == RenderType::None)
		return Status::NotInitializedError;

	MH_STATUS st = MH_DisableHook(_target);
	if (st != MH_OK && st != MH_ERROR_FUNCTION_NOT_FOUND)
		return Status::UnknownError;

	st = MH_RemoveHook(_target);
	if (st != MH_OK && st != MH_ERROR_FUNCTION_NOT_FOUND)
		return Status::UnknownError;
...
kiero::Status::Enum kiero::unbindImport(const char* _moduleName, const char* _procName)
{
	assert(_moduleName != NULL && _procName != NULL);
