...
namespace kiero
{
	...
	Status::Enum resolveModuleHandle(const char* _moduleName, void** _outBuffer);

	Status::Enum bind(uint16_t index, void** original, void* function);
	void unbind(uint16_t index);

	Status::Enum bindFunction(void* target, void** original, void* function);
	Status::Enum unbindFunction(void* target);

	Status::Enum bindImport(const char* moduleName, const char* procName, void** original, void* function);
	Status::Enum unbindImport(const char* moduleName, const char* procName);
